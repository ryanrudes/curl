...
def parse_args():
    ...
    parser.add_argument('--no-curl', dest='curl', action='store_false')
...
def evaluate(env, agent, video, num_episodes, L, step, args):
    ...
    def run_eval_loop(sample_stochastically=True):
        ...
        for i in range(num_episodes):
            ...
            videos.append(wandb.Video(np.array(video.frames).transpose(0, 3, 1, 2), fps = 60, caption = f"{episode_reward:.4f}", format = "mp4"))
        ...
        wandb.log({'eval/rollouts': videos}, step = step)
...
def make_agent(obs_shape, action_shape, args, device):
    if args.agent == 'curl_sac':
        return CurlSacAgent(
            ...
            contrastive_learning=args.curl,
        )
    else:
        assert 'agent is not supported: %s' % args.agent

"""
class ImageObservation(gymnasium.Wrapper):
    def __init__(self, env):
        super().__init__(env)
        self.observation_space = gymnasium.spaces.Box(low = 0, high = 255, shape = (3, 100, 100), dtype = np.uint8)
    ...
    def step(self, action):
        ...
        return self.observe(), reward, terminated or truncated, info
"""
...
def main():
    ...
    #env = gymnasium.make("Reacher-v4", render_mode = "rgb_array", height = 100, width = 100)
    ...
    #wandb.tensorboard.patch(root_logdir=args.work_dir)
    run = wandb.init(
        entity = "ryan-steven",
        project = "CURL",
        name = f"{args.domain_name}:{args.task_name}",
