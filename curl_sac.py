...
"""
Key differences:
- action repeat = 4 (not 1)
- unified loss vs separate optimizers
"""
...
class Actor(nn.Module):
    ...
    def log(self, L, step, log_freq=LOG_FREQ):
        ...
        #for k, v in self.outputs.items():
            ...
            #wandb.log({'train_actor/%s_hist' % k: wandb.Histogram(v)}, step = step)
...
class Critic(nn.Module):
    ...
    def log(self, L, step, log_freq=LOG_FREQ):
        ...
        #for k, v in self.outputs.items():
            ...
            #wandb.log({'train_critic/%s_hist' % k: wandb.Histogram(v)}, step = step)
...
class CurlSacAgent(nn.Module):
    ...
    def update(self, replay_buffer, L, step):
        ...
        if self.contrastive_learning and step % self.cpc_update_freq == 0 and self.encoder_type == 'pixel':
